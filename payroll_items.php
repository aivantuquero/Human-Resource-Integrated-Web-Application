...
?>
						...
						<table id="table" class="table table-bordered table-striped">
							...
							<tbody>
								<?php
									...
									$payroll=$conn->query("SELECT p.*,concat(e.lastname,', ',e.firstname,' ',e.middlename) as ename,e.employee_no FROM payroll_items p inner join employee e on e.id = p.employee_id where p.payroll_id = ".$_GET['id']) or die(mysqli_error($conn));
								...
								<tr>
									...
									<td>
										<center>
										 ...
										 <button class="btn btn-sm btn-outline-primary print_btn_employee" data-id2="<?php echo $row['id']?>" type="button"><span class="fa fa-print"></span> Payslip</button>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('.print_btn_employee').click(function(){
				var $id=$(this).attr('data-id2');

				var nw = window.open("print_employee_payroll.php?id="+$id,"_blank","height=500,width=800")
				setTimeout(function(){
					nw.print()
					setTimeout(function(){
						nw.close()

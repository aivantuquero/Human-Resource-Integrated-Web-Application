...
<div class="container-fluid">
	<form action="" id="employee-deduction">
		...
		<div class="row form-group">
			...
			<div class="col-md-5" id="loan-with-interest-amount">
				<label for="" class="control-label">Total Loan Amount</label>
				<input type="number" id="total-loan-input" disabled placeholder="Total loan with interest" class="form-control text-right" step="any" >
				<p class="font-italic text-secondary h6">*According to company policy, the sum will be subject to a 6-month payment period.</p>
			</div>	
...
<script>
	...
	$('#loan-with-interest-amount').hide()
	...
	$('#type').change(function(){
		...
		$('#amount').val(null)
		$('#total-loan-input').val(null)
	})
	$('#deduction_id').change(function(){
		if($(this).val() == 5){
			$('#loan-with-interest-amount').show()
		}else{
			$('#loan-with-interest-amount').hide()
		}
	})
	$('#amount').on("input", ()=>{
		if($('#type').val() == 1){
			var interest_rate = $('#amount').val()*0.02
			$('#total-loan-input').val((parseFloat(interest_rate) + parseFloat($('#amount').val())) / 6)
		}
		if($('#type').val() == 2){
			var interest_rate = $('#amount').val()*0.02
			$('#total-loan-input').val(((parseFloat(interest_rate) + parseFloat($('#amount').val()))/ 6)/2)
	...
	$('#add_list').click(function(){
		var deduction_id = $('#deduction_id').val(),
			...
			// deduction is is on 'loan' then get the loan amount from total loan input text box
			if(deduction_id == 5){
				amount = $('#total-loan-input').val()
			}

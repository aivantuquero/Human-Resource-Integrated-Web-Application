...
Class Action {
	...
	function save_employee(){
		...
		if(empty($id)){
			...
			$data2 .=", username='$e_num' ";


			$this->db->query("INSERT INTO users set ".$data2);
		}else{
			$save = $this->db->query("UPDATE employee set ".$data." where id=".$id);
			$this->db->query("UPDATE tasks set ".$data2." where id=".$id);
		}
		if($save)
			return 1;
	}
	...
	function save_ticket(){
		...
		// get last taskid
		$lastTask = $this->db->query("SELECT task_id FROM tasks order by id desc limit 1")->fetch_array();
		$lastTask['task_id'];

		$task_id = $lastTask['task_id'] + 1;

		$data =" task_title = '$title' ";
		$data .=", task_information = '$information' ";
		$data .=", task_id = '$task_id' ";
		...
		if(empty($id)){
			...
			foreach($selected_employee as $emp){
	
				$save = $this->db->query("INSERT INTO tasks set ".$data.", employee_id= '".$emp."' ");
			}
		}else{
			$save = $this->db->query("UPDATE tasks set ".$data." where id=".$id);
		}
	...
	function update_ticket(){
		...
		$save = $this->db->query("UPDATE tasks SET ".$data." WHERE task_id = ".$task_id." AND employee_id = ".$employeeid);
	...
	function check_employee(){
		...
		$task = $this->db->query("SELECT * FROM tasks WHERE employee_id = ".$employeeid." AND task_id = ".$task_id."")->fetch_array();
	...
	function delete_ticket(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM tasks where id = ".$task_id);
		if($delete)
			return 1;
	}
	...
	function calculate_payroll(){
		...
		//get attendance
		$att=$this->db->query("SELECT * FROM attendance where date(datetime_log) between '".$pay['date_from']."' and '".$pay['date_to']."' order by UNIX_TIMESTAMP(datetime_log) asc  ") or die(mysqli_error($this->db));
		...
		$deductions = $this->db->query("SELECT * FROM employee_deductions where (`type` = '".$pay['type']."' or (date(effective_date) between '".$pay['date_from']."' and '".$pay['date_to']."' ) ) ");
		$allowances = $this->db->query("SELECT * FROM employee_allowances where (`type` = '".$pay['type']."' or (date(effective_date) between '".$pay['date_from']."' and '".$pay['date_to']."' ) ) ");
		...
		// loop for every each employee
		while($row =$employee->fetch_assoc()){
			$salary = $row['salary'];
			$daily = $salary / 26;
			$min = (($salary / 26) / 8) /60; # calculates the per-minute salary
			$absent = 0;
			$late = 0;
			$dp = 26 / $pay['type'];
			$present=0;
			...
			for($i = 0; $i < $calc_days;$i++){
				...
				// 10_2023-05-01: array(1)
				// 	log: array(4)
				// 		1: "2023-05-01 06:00:00"
				// 		2: "2023-05-01 11:00:00"
				// 		3: "2023-05-01 12:00:00"
				// 		4: "2023-05-01 16:00:00"
					...
					if(isset($attendance[$row['id']."_".$dd]['log'][1]) && isset($attendance[$row['id']."_".$dd]['log'][2])){
        				...
        				$att_mn =floor($att_mn  /60 ); //converts the attendance value to minutes
        				$net += ($att_mn * $min);
        				$late += (240 - $att_mn);
        				$present += .5;
			...
			// if the date is 15
			if(date('d', strtotime($pay['date_to'])) == '15'){
				$philhealth = round(($net * 0.045), 2);
				$sss = round(($net * 0.14), 2);
				$pagibig = round(($net * 0.01),2);

				$net -= ($philhealth + $sss + $pagibig);
